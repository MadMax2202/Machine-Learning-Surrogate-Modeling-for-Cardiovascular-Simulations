function run_10k_conditional_sampling_pediatric_noVAD_linear(samples_path, results_path)

    clearvars -except samples_path results_path;
    clc;

    fprintf("\nStarting conditional sampling 10k MATLAB chunk with resume support...\n");
    ...
    for i = 1:n_remaining
        ...
        fprintf("\nRunning remaining %d/%d | simulation_id = %d | patient_id = %d | variation_id = %d\n", ...
            i, n_remaining, sample_row.simulation_id, sample_row.patient_id, sample_row.variation_id);

        try
            raw_result = run_one_case_conditional_sampling_pediatric_noVAD_linear(sample_row, model);
    ...
    fprintf("\nFinished/resumed conditional sampling chunk.\n");
